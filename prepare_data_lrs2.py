...
def create_wsj_csv(datapath, savepath):
    ...
    for set_type in ["tr", "cv", "tt"]:
        mix_path = os.path.join(datapath, "wav16k/min/" + set_type + "/mix/")
        s1_path = os.path.join(datapath, "wav16k/min/" + set_type + "/s1/")
        s2_path = os.path.join(datapath, "wav16k/min/" + set_type + "/s2/")
        ...
        with open(savepath + "/lrs2_" + set_type + ".csv", "w") as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
            writer.writeheader()
            for i, (mix_path, s1_path, s2_path) in enumerate(
                zip(mix_fl_paths, s1_fl_paths, s2_fl_paths)
